...
interface MembershipsCrudLabels {
  create: string;
  disable: string;
  planCode: string;
  save: string;
...
export function MembershipsCrud({
  rows,
  disabled = false,
  labels,
}: {
  disabled?: boolean;
  labels: MembershipsCrudLabels;
  rows: MembershipRow[];
}) {
  ...
  return (
    <div className="space-y-3">
      <form action={create} className="grid gap-2 rounded-ds-radius-md border border-ds-border bg-ds-bg/50 p-3 md:grid-cols-3 lg:grid-cols-[1fr_140px_140px_140px_140px_auto]">
      ...
      {rows.map((row) => (
        <form key={row.id} action={(fd) => update(row.id, fd)} className="grid gap-2 rounded-ds-radius-md border border-ds-border bg-ds-surface-2/35 p-3 md:grid-cols-3 lg:grid-cols-[1fr_140px_140px_140px_140px_auto_auto]">
          ...
          <Button disabled={pending || disabled} type="button" variant="destructive" onClick={() => remove(row.id)}>{labels.disable}</Button>
        </form>
      ))}
    </div>
  );

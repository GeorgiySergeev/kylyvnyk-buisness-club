...
export function IntroductionForm({ businesses, labels, locale }: IntroductionFormProps) {
  ...
  return (
    <form className="space-y-5" noValidate onSubmit={onSubmit}>
      {errors.root?.message ? (
        <div className="alert alert-error rounded-box" role="alert">
          <span>{errors.root.message}</span>
        </div>
      ) : null}

      {successMessage ? (
        <div className="alert alert-success rounded-box" role="status">
          <span>{successMessage}</span>
        </div>
      ) : null}

      <div className="space-y-2">
        <label className="text-sm font-semibold text-foreground" htmlFor="targetBusinessId">
        ...
        <select
          ...
          className="select select-bordered min-h-11 w-full rounded-field bg-background"
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div className="space-y-2">
          <label className="text-sm font-semibold text-foreground" htmlFor="clientName">
            {labels.clientName}
          </label>
          <input
            aria-describedby={errors.clientName ? 'clientName-error' : undefined}
            aria-invalid={Boolean(errors.clientName)}
            className="input input-bordered min-h-11 w-full rounded-field bg-background"
            disabled={pending}
            id="clientName"
            type="text"
            {...register('clientName')}
        ...
        <div className="space-y-2">
          <label className="text-sm font-semibold text-foreground" htmlFor="clientContact">
          ...
          />
          <p className="text-xs leading-5 text-muted-foreground" id="clientContact-help">
      ...
      <div className="space-y-2">
        <label className="text-sm font-semibold text-foreground" htmlFor="message">
          {labels.message} <span className="text-muted-foreground">({labels.optional})</span>
        </label>
        <textarea
          aria-describedby="message-help message-error"
          aria-invalid={Boolean(errors.message)}
          className="textarea textarea-bordered min-h-28 w-full rounded-field bg-background"
          disabled={pending}
          id="message"
          {...register('message')}
        />
        <p className="text-xs leading-5 text-muted-foreground" id="message-help">
          {labels.messageHelp}
        </p>
